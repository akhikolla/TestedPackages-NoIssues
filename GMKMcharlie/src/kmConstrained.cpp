...
struct assignMedoid
{
  ...
  assignMedoid(valtype *clusterWeightUpperLimit,
    ...
    eventV(eventV), eventCluster(eventCluster),
    D(D), Dorder(Dorder), DorderAux(DorderAux)
  {
...
inline void updateCentroid(
    indtype whichCluster, centroid<indtype, valtype> *C,
...
struct updateCentroidV: public Worker
{
  ...
  void operator() (std::size_t st, std::size_t end)
  {
    for(;;)
    {
      ...
      updateCentroid<indtype, valtype, beta> (
          objI, &centroidV[0], eventVbegin, eventCentroidV, clusterChanged);
    }
  }
  ...
  updateCentroidV(vec<centroid<indtype, valtype> > &centroidV,
                  event<indtype, valtype> *eventVbegin,
                  eventCentroidIndex<indtype> *eventCentroidV,
                  vec<bool> &clusterChanged, indtype maxCore):
...
struct compDfun: public Worker
{
             ...
             vec<bool> &clusterChanged)
  {
    ...
    d = minkD<event<indtype, valtype>, centroid<indtype, valtype>, 0,
...
inline List KMconstrainedCppTemplate
  (
      NumericMatrix X,
...
{
  double *clusterWeightUpperBound = &maxClusterWeights[0];
  INT d = X.nrow();


  vec<KMconstrained::event<int, double> > eventV(X.ncol());
  ...
  vec<KMconstrained::centroid<int, double> >
    centroidV(centroid.ncol());
  ...
  if(true) // Initialize centroidV.
  {
    ...
    {
      centroidV[i].loss = &centroidContainer[0] + i * d;
      centroidV[i].size = d;
      if(beta == -1) // consine dissimilarities.
      {
  ...
  vec<KMconstrained::eventCentroidIndex<int> >
  ...
  if(true) // Clustering
  {
    ...
    while(true)
    {
      KMconstrained::compDfun<int, double, beta> (
          centroidV.size(), eventV.size(), minkP,
          &D[0], &eventV[0], &centroidV[0], clusterChanged, maxCore);


      KMconstrained::assignMedoid<int, orderIndtype, double> T(
          clusterWeightUpperBound, &eventV[0], eventCentroidIndexV,
      ...
      if(verbose)
      {
        ...
        {
          Rcout << "total in-cluster distance before root = ";
          Rcout << T.sumOfD;
          Rcout << ", ";
        }
        if(howManyDiff != 0)
      ...
      KMconstrained::updateCentroidV<int, double, beta> (
         ...
         eventCentroidIndexV[i - 1].centroidID)
      {
        ...
        for(int u = 0, uend = i - j, v = j; u < uend; ++u, ++v)
        {
          ...
          eventToCentroidDistance[u] = minkD<
            KMconstrained::event<int, double>,
            KMconstrained::centroid<int, double>, 0,
            int, double, beta, true> (
            eventV[eventID[u]], centroidV[k], minkP);
          ++eventID[u];
        }
        rst[k] = List::create(
          Named("centroid") =
            NumericVector(centroidV[k].loss, centroidV[k].loss + centroidV[k].size),
...
inline List KMconstrainedCppBetaRealized
  (
      NumericMatrix X,
      NumericMatrix centroid,
      NumericVector Xw,
...
{
  if(minkP == 2) // beta == 2
    return KMconstrainedCppTemplate<orderIndtype, 2> (
        X, centroid, Xw, maxClusterWeights, minkP, maxCore, convergenceTail,
        tailConvergedRelaErr, maxIter, paraSortInplaceMerge, verbose);


  if(minkP == 1) // l1 norm
    return KMconstrainedCppTemplate<orderIndtype, 1> (
        X, centroid, Xw, maxClusterWeights, minkP, maxCore, convergenceTail,
        tailConvergedRelaErr, maxIter, paraSortInplaceMerge, verbose);


  if(minkP == 0) // max norm.
    return KMconstrainedCppTemplate<orderIndtype, 0> (
        X, centroid, Xw, maxClusterWeights, minkP, maxCore, convergenceTail,
        tailConvergedRelaErr, maxIter, paraSortInplaceMerge, verbose);
  ...
  if(std::abs(int(minkP) / minkP - 1) < 1e-10 and minkP >= 3 and minkP <= 35)
    return KMconstrainedCppTemplate<orderIndtype, 3> (
        X, centroid, Xw, maxClusterWeights, minkP, maxCore, convergenceTail,
        tailConvergedRelaErr, maxIter, paraSortInplaceMerge, verbose);
  ...
  return KMconstrainedCppTemplate<orderIndtype, 4> (
      X, centroid, Xw, maxClusterWeights, minkP, maxCore, convergenceTail,
      tailConvergedRelaErr, maxIter, paraSortInplaceMerge, verbose);
}
...
)
{
  ...
  if(Dsize < 255)
    return KMconstrainedCppBetaRealized<unsigned char>
    (
        X, centroids, Xw, clusterWeightUpperBound,
  ...
  if(Dsize < 65535)
    return KMconstrainedCppBetaRealized<unsigned short>
    (
        X, centroids, Xw, clusterWeightUpperBound,
        minkP, maxCore, convergenceTail,
        tailConvergedRelaErr,
  ...
  if(Dsize < 4294967295)
    return KMconstrainedCppBetaRealized<unsigned>
    (
        X, centroids, Xw, clusterWeightUpperBound,
        minkP, maxCore, convergenceTail,
        tailConvergedRelaErr,
  ...
  return KMconstrainedCppBetaRealized<std::size_t>
    (
        X, centroids, Xw, clusterWeightUpperBound,
        minkP, maxCore, convergenceTail,
        tailConvergedRelaErr,
