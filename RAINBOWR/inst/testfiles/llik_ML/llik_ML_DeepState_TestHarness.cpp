// AUTOMATICALLY GENERATED BY RCPPDEEPSTATE PLEASE DO NOT EDIT BY HAND, INSTEAD EDIT
// llik_ML_DeepState_TestHarness_generation.cpp and llik_ML_DeepState_TestHarness_checks.cpp

#include <fstream>
#include <RInside.h>
#include <iostream>
#include <RcppDeepState.h>
#include <qs.h>
#include <DeepState.hpp>

double llik_ML(double n, double yPy, double lnHinv);

TEST(RAINBOWR_deepstate_test,llik_ML_test){
  RInside R;
  std::cout << "input starts" << std::endl;
  NumericVector n(1);
  n[0]  = RcppDeepState_double();
  qs::c_qsave(n,"/home/akhila/fuzzer_packages/fuzzedpackages/RAINBOWR/inst/testfiles/llik_ML/inputs/n.qs",
		"high", "zstd", 1, 15, true, 1);
  std::cout << "n values: "<< n << std::endl;
  NumericVector yPy(1);
  yPy[0]  = RcppDeepState_double();
  qs::c_qsave(yPy,"/home/akhila/fuzzer_packages/fuzzedpackages/RAINBOWR/inst/testfiles/llik_ML/inputs/yPy.qs",
		"high", "zstd", 1, 15, true, 1);
  std::cout << "yPy values: "<< yPy << std::endl;
  NumericVector lnHinv(1);
  lnHinv[0]  = RcppDeepState_double();
  qs::c_qsave(lnHinv,"/home/akhila/fuzzer_packages/fuzzedpackages/RAINBOWR/inst/testfiles/llik_ML/inputs/lnHinv.qs",
		"high", "zstd", 1, 15, true, 1);
  std::cout << "lnHinv values: "<< lnHinv << std::endl;
  std::cout << "input ends" << std::endl;
  try{
    llik_ML(n[0],yPy[0],lnHinv[0]);
  }
  catch(Rcpp::exception& e){
    std::cout<<"Exception Handled"<<std::endl;
  }
}
