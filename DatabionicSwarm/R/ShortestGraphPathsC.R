...
# Dijkstra, E. W.: A note on two problems in connexion with graphs, Numerische mathematik, Vol. 1(1), pp. 269-271. 1959.
  ...
  if(!isSymmetric(Adj)) stop('Adj is not symmetric, maybe a directed instead of an undirected graph was used?')
  
  if(!isSymmetric(Cost)) warning('Cost is not symmetric.')
