...
server.mfssa <- function(input, output, clientData, session) {
  ...
  Tr1 <- function(tau, t){
    ...
    if ("f2"%in%input$model) tr1 <- tr1 + ( cos(2*pi*input$a2.f*t) * exp(1-tau^2) + sin(2*pi*input$a2.f*t) * sin(pi*tau) ); return(tr1)
  }
  Tr2 <- function(tau, t){
  ...
  simulate <- function() {
    if (is.null(input$a1.f) || ("f2"%in%input$model && is.null(input$a2.f))) return()
    tau <- seq(0, 1, length = T); t <- 1:input$t.len; noise <- list()
    ...
    for (i in 1:length(Trs)) {
      ...
      if (input$noise.t=="far1") {
        ...
        Z[1, ] <- 0; noise[[i]] <- apply(Z, 2, cumsum)
        ...
        basis.noise <- rfar(input$t.len,input$noise.p,psi,Eps,basis.Z); noise[[i]] <- eval.fd(tau, basis.noise)
      }
    }
  ...
  observeEvent(input$dmd.uf, {updateTabsetPanel(session, "Panel", selected = "Data"); updateCheckboxGroupInput(session, "s.plot", selected="")})
  ...
  run_fpca <- function(){#eventReactive(input$run.fpca, {
    withProgress(message = '(D)FPCA: Running', value = 0, {
      ...
      tau <- seq(0, 1, length = nrow(iTs()[[1]])); Y <- smooth.basis(tau, iTs()[[ifelse(is.null(input$var.which),1,as.numeric(input$var.which))]], bas.fssa)$fd
  ...
  output$s.choice <- renderUI({
    ...
    if (!length(iXs())) {
      ...
      Xs[[4]] <- Xs[2:3]; names(Xs) <- c(names(Xs[1:3]),"xDI"); iXs(Xs)
    }
  ...
  output$model <- renderUI({
    if (input$f.choice!="sim") return();  choices <- c("A.1t(w1,l) F1 +" = "f1", "A.2t(w2) F2" = "f2")
  ...
  output$a1.f <- renderUI({
    if (input$f.choice!="sim" || !("f1" %in% input$model)) return();
  ...
  output$a2.f <- renderUI({
    if (input$f.choice!="sim" || !("f2" %in% input$model)) return();
  ...
  output$ts.selected = renderText({
    ...
    if (input$f.choice=="upload") {
      ...
      if (!input$header && !is.numeric(Ts)) {
        headers <- as.factor(Ts[1,]); TS <- list();
      ...
      Ts <- iXs()[[i]]; if (is.matrix(Ts)) Ts <- list(Ts);
    ...
    if ("dmd" %in% input$dmd.uf) {for (i in length(Ts)) {Ts[[i]] <- Ts[[i]]-mean(Ts[[i]]); if (input$f.choice=="sim") simul$Trs[[i]] <- simul$Trs[[i]]-mean(simul$Trs[[i]])}}
  ...
  output$var.which <- renderUI({
    if ((input$f.choice=="upload" && is.null(input$file)) || (input$f.choice=="sim" && !length(input$model)) || is.null(input$desc)) return();
    if (!input$desc%in%c("ts","gcv","ssa.reconst","mfssa.reconst","mfssa.singF") || length(iTs())==1) return()
    choic <- as.character(1:length(iTs())); names(choic) <- names(iTs())
  ...
  output$s.plot <- renderUI({
    ...
    if ("uf" %in% input$dmd.uf) {choices <- append(choices,"fssa",6); names(choices)[7] <- "Functional SSA"};
  ...
  fda.gcv <- eventReactive(input$run.fda.gcv, {
    withProgress(message = 'FDA.GCV: # of Basis', value = 0, { GCV <- NULL; df <- min(df,nrow(iTs()[[1]]))
  ...
  output$res.plot <- renderPlot({
    ...
    if (substr(input$desc,1,5)=="mfssa" || sum(c("mfssa","fssa","ssa")%in%input$s.plot) || substr(input$desc,1,3)=="ssa") {
      ...
      for (i in input$sg[1]:input$sg[2]) {
        ...
        mQf$coefs <- mQf$coefs + sr$mQf[[i]][[var.which]]$coefs
      }
      if ("uf" %in% input$dmd.uf) uQ <- eval.fd(seq(0, 1, length = nrow(Ts)),uQf)
      mQ <- eval.fd(seq(0, 1, length = nrow(Ts)),mQf)
    }
    ...
    if (substr(input$desc,1,5)=="mfssa") {
      ...
      if ("fssa" %in% input$s.plot) {for (i in indx) points(uQ[,i],type="l",col=7)}
      ...
      if ("tf" %in% input$s.plot) {for (i in indx) points(iTrs()[[var.which]][,i],type="l",col=2)}
