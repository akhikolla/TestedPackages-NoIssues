...
result <- as.numeric(HRM::hrm_test(value~Var1*Var2*Var3*Var4*Var5, data = df, subject = "subject", nonparametric = TRUE, np.correction = FALSE)$result[1, 2:5])
result2 <- as.numeric(HRM::hrm_test(value2~Var1*Var2*Var3*Var4*Var5, data = df, subject = "subject", nonparametric = TRUE, np.correction = FALSE)$result[1, 2:5])
result_hrm <- HRM::hrm_test(value~Var1*Var2*Var3*Var4*Var5, data = df, subject = "subject", nonparametric = TRUE, np.correction = FALSE)
